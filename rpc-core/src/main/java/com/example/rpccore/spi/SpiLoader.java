package com.example.rpccore.spi;
...
public class SpiLoader {
     ...
     * 系统SPI目录
     */
    private static final String RPC_SYSTEM_SPI_DIR = "META-INF.services.system";

    /**
     * 用户自定义SPI目录
     */
    private static final String RPC_CUSTOM_SPI_DIR = "META-INF.services.custom";
    ...
    public static <T> T getInstance(Class<?> tClass,String key) {
        ...
        if(!instanceCache.containsKey(implClassName)) {
            try {
                instanceCache.put(implClassName,
                        implClass.newInstance());
            } catch (InstantiationException | IllegalAccessException e) {
                String errorMeg = String.format("%s 类实例化失败",implClassName);
                throw new RuntimeException(errorMeg,e);
            }
        }
